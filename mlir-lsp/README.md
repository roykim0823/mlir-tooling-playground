...
hint until [`../mlir-tablegen/capstone-toy/`](../mlir-tablegen/capstone-toy/README.md)
is built.
...
`--mlir-print-op-generic` shows ([debugging track](../mlir-debugging/README.md)),
...
```yaml
--- !FileInfo:
  filepath: ".../mlir-tablegen/capstone-toy/include/Toy/ToyOps.td"
  includes: ".../capstone-toy;/opt/homebrew/opt/llvm@20/include;.../capstone-toy/include;.../capstone-toy/build/include"
```
...
`mlir-tablegen/capstone-toy/build/tablegen_compile_commands.yml`. Pass it with
...
```bash
./make-session.py ../mlir-tablegen/capstone-toy/include/Toy/ToyOps.td --hover 84:7 --definition 84:14 --references 84:14 > /tmp/td.session
tblgen-lsp-server --lit-test --tablegen-compilation-database=../mlir-tablegen/capstone-toy/build/tablegen_compile_commands.yml \
...
```bash
./make-session.py ../mlir-tablegen/capstone-toy/include/Toy/ToyPatterns.pdll --hover 27:18 --hover 31:27 --definition 31:50 > /tmp/pdll.session
mlir-pdll-lsp-server --lit-test --pdll-compilation-database=../mlir-tablegen/capstone-toy/build/pdll_compile_commands.yml \
...
ODS) — the [PDLL track](../mlir-pdll/README.md) is much nicer to follow with
it running.
...
[`tools/toy-lsp-server.cpp`](../mlir-tablegen/capstone-toy/tools/toy-lsp-server.cpp):
...
```bash
../mlir-tablegen/capstone-toy/build/toy-lsp-server --lit-test < sessions/toy.session 2>/dev/null | ./summarize.py
```
```
...
[`test/lsp.test`](../mlir-tablegen/capstone-toy/test/lsp.test): the RUN line
...
```jsonc
{
  ...
  "mlir.tablegen_compilation_databases": [
    "${workspaceFolder}/mlir-tablegen/capstone-toy/build/tablegen_compile_commands.yml"
  ],
  "mlir.pdll_server_path": "/opt/homebrew/opt/llvm@20/bin/mlir-pdll-lsp-server",
  "mlir.pdll_compilation_databases": [
    "${workspaceFolder}/mlir-tablegen/capstone-toy/build/pdll_compile_commands.yml"
  ],
...
`${workspaceFolder}/mlir-tablegen/capstone-toy/build/toy-lsp-server` instead
...
vim.api.nvim_create_autocmd("FileType", { pattern = "tablegen", callback = function()
  vim.lsp.start({ name = "tblgen", cmd = { "tblgen-lsp-server",
    "--tablegen-compilation-database=" .. vim.fn.getcwd() .. "/mlir-tablegen/capstone-toy/build/tablegen_compile_commands.yml" } })
end })
```
