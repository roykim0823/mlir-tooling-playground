...
```bash
mlir-opt --load-dialect-plugin=../../../mlir-capstone/build/ToyPlugin.dylib --show-dialects | tr ',' '\n' | grep toy
mlir-opt --load-pass-plugin=../../../mlir-capstone/build/ToyPlugin.dylib --list-passes | grep -A1 -- '--toy-fold'
```
...
[`mlir-tools/reduce/`](../reduce/README.md) track.
...
TOY=../../../mlir-capstone/include          # -I for `include "Toy/ToyInterfaces.td"`
...
(`-x=mlir`, the PDL it compiles to, is in the [PDLL track](../../mlir-patterns/pdll/README.md)):
