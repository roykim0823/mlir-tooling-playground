...
[`../mlir-tablegen/capstone-toy/`](../mlir-tablegen/capstone-toy/README.md) is
...
dialect. The capstone's [`tools/toy-reduce.cpp`](../mlir-tablegen/capstone-toy/tools/toy-reduce.cpp):
...
Its tester,
[`test/Inputs/interesting.sh`](../mlir-tablegen/capstone-toy/test/Inputs/interesting.sh),
says "after `-toy-fold` a `toy.sub` remains":

```bash
toy-reduce ../../mlir-tablegen/capstone-toy/test/reduce.mlir \
  -reduction-tree='traversal-mode=0 test=../../mlir-tablegen/capstone-toy/test/Inputs/interesting.sh' -o toy.mlir
```
```mlir
module {
...
test, [`test/reduce.mlir`](../mlir-tablegen/capstone-toy/test/reduce.mlir): a
...
1. **Capture.** A failing pipeline gives you the IR just before the failing
   ...
   ([debugging track](../mlir-debugging/README.md), Section 3). Strip the
